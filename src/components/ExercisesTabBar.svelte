<script>
	...
	// Function to handle central button click - now starts training
...
{#if $tabBarVisible}
	<div class="tab-bar">
		...
		<!-- Central cutout button - now starts training -->
		<div class="central-button-container" on:click={onCentralButtonClick} class:disabled={disabled}>
			<div class="central-button-cutout"></div>
			<div class="central-button" class:disabled={disabled}>
				<div class="central-tab-icon">
					<!-- Central icon - play icon for starting training -->
					<span class="central-button-text">{disabled ? 'Select' : 'Ready'}</span>
				</div>
			</div>
		</div>
...
<style>
	...
	.central-button {
		...
		border-radius: 50%;
		background: linear-gradient(135deg, #41B6E6, #00BFFF); /* Blue gradient for start button */
		display: flex;
		align-items: center;
		justify-content: center;
		z-index: 2;
	...
	/* Disabled state for central button */
	.central-button.disabled {
		background: linear-gradient(135deg, #666666, #999999);
		cursor: not-allowed;
		opacity: 0.6;
	}
	
	.central-button.disabled:hover {
		transform: none;
		box-shadow: 
			0 4px 10px rgba(0, 0, 0, 0.3),
			inset 0 2px 2px rgba(255, 255, 255, 0.3);
	...
	.central-button.disabled:active {
		transform: none;
		box-shadow: 
			0 4px 10px rgba(0, 0, 0, 0.3),
			inset 0 2px 2px rgba(255, 255, 255, 0.3);
	...
	.central-button-container.disabled {
		cursor: not-allowed;
	}
